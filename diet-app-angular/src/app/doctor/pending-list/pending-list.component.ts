...
export class PendingListComponent implements OnInit, AfterContentChecked {
  ...
  private initForm() {
    this.submitUserForm = new FormGroup({
      correctedValue: new FormControl(null, [Validators.required, Validators.minLength(1)]),
  ...
  onSubmit() {
    this.pendingService.submitPatient(this.idPatient, 12, this.submitUserForm.value.pal, this.submitUserForm.value.correctedValue).subscribe((response) => {
      console.log(response);
